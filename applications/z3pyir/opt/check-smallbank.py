...
def GenArgv():
    ...
    builder.AddArgv('custId1',ArgvBuilder.Type.INT)
...
class Op_TransactSavings_run_60():
    ...
    def cond0(self, state, argv):
        ...
        return And((Not(state['TABLE_ACCOUNTS'].notNil({'name' : custName}) == False)),(Not(state['TABLE_SAVINGS'].notNil({'custid' : custId}) == False)),(Not((state['TABLE_SAVINGS'].get({'custid' : custId}, 'bal') - amount) < 0)))
...
class Op_Amalgamate_run_81():
    ...
    def cond0(self, state, argv):
        ...
        custId0 = argv['Amalgamate_run_81']['custId0']
        total = argv['Amalgamate_run_81']['total']
        return And((Not(state['TABLE_ACCOUNTS'].notNil({'custid' : custId0}) == False)),(Not(state['TABLE_CHECKING'].notNil({'custid' : custId0}) == False)),((total >= 0)),(((state['TABLE_CHECKING'].get({'custid' : custId0}, 'bal') - total) >= 0)))
...
class Op_SendPayment_run_59():
    ...
    def cond0(self, state, argv):
        ...
        return And((Not(state['TABLE_ACCOUNTS'].notNil({'custid' : destAcct}) == False)),(Not(state['TABLE_ACCOUNTS'].notNil({'custid' : destAcct}) == False)),(Not(state['TABLE_CHECKING'].notNil({'custid' : sendAcct}) == False)),((amount >= 0)),((state['TABLE_CHECKING'].get({'custid' : sendAcct}, 'bal') >= amount)))
